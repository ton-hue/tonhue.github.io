...
vector<Bid> loadBids(string csvPath) {
	...
	for (unsigned int i = 0; i < file.rowCount(); i++) {
		...
		bids.title = file[i][0];
		bids.fund = file[i][8];
		bids.amount = strToDouble(file[i][4], '$');

		cout << "Title: " << bids.title << ", Funds: " << bids.fund << ", Amount: " << bids.amount << endl;

		bidnums.push_back(bids);
